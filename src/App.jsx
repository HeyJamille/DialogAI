...
const App = () => {
  const [messages, setMessages] = useState([]); // Estado para mensagens da Home
  const [sidebarMessages, setSidebarMessages] = useState([]); // Estado para mensagens do Sidebar
  ...
  return (
    <article className="flex flex-row h-screen bg-[#121212]">
      <div className="hidden md:flex md:z-20 md:top-0">
        <SideBar messages={sidebarMessages} /> {/* Passa a lista de mensagens para o SideBar */}
      </div>
      <div className="flex flex-col justify-between w-full">
        <Header clearMessages={clearMessages} handleSendSidebar={handleSendSidebar} />
